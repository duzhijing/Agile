...
namespace AgileNewsService
{
    ...
    public class NewsService : INewsService
    {
        ...
        public News GetAloneNews(int newsId)
        {
            ...
            {
                conn.Open();
                string sql = string.Format(@"select * from news where NEWSID=:NewsID");
                var result = conn.Query<News>(sql, new { NewsID = newsId });
        ...
        public int NewsAdd(News news)
        {
            using (OracleConnection conn = DapperHelper.GetConnString())
            {
                ...
                //return i;
                string sql = @"insert into News(NewsTitle,NewsPublisher,NewsPublishDate,NewsDetails,NewsSort,NewsReader,NewsUpdateDate,NewsTypeName,IsDelete,LikeTablePid,Category) values(:NewsTitle,:NewsPublisher,:NewsPublishDate,:NewsDetails,:NewsSort,:NewsReader,:NewsUpdateDate,:NewsTypeName,:IsDelete,:LikeTablePid,:Category)";
        ...
        public int NewsUpdate(News news)
        {
            ...
            {
                conn.Open();
                string sql = @"Update News set  NewsTitle=:NewsTitle,NewsPublisher=:NewsPublisher,NewsPublishDate=:NewsPublishDate,NewsDetails=:NewsDetails,NewsSort=:NewsSort,NewsReader=:NewsReader,NewsUpdateDate=:NewsUpdateDate,NewsTypeName=:NewsTypeName,IsDelete=:IsDelete where LikeTablePid=:LikeTablePid";
