...
namespace AgileNewsService
{
    ...
    public class RoleService : IRoleService
    {
        public int AddRole(Role r)
        {
            ...
            {
                string executeSql = @" INSERT INTO Role (RoleName,PowerId,PowerName,CreateTime) VALUES (:RoleName,:PowerId,:PowerName,:CreateTime)";
                r.CreateTime = Convert.ToString(System.DateTime.Now);
                var Collectlist = new { RoleName = r.RoleName, PowerId = r.PowerId, PowerName = r.PowerName, CreateTime = r.CreateTime };
                ...
                if (result > 0)
                {
                    ...
                    for (int i = 0; i < PowerId.Length; i++)
                    {
                        ...
                        string sql1 = @"insert into RoleAction (RoleId,PowerId,CreateTime) VALUES (:RoleId,:PowerId,:CreateTime)";
        ...
        public int UpdateRole(Role role)
        {
            using (OracleConnection conn = DapperHelper.GetConnString())
            {
                ...
                role.ModifyTime = Convert.ToString(System.DateTime.Now);
                ...
                if (result > 0)
                {
                    ...
                    for (int i = 0; i < PowerIds.Length; i++)
                    {
                        ...
                        string sql1 = @"insert into RoleAction (RoleId,PowerId,ModifyTime) VALUES (:RoleId,:PowerId,:ModifyTime)";
                        int result1 = conn.Execute(sql1, roleAction);
                    }
                }
                return result;
